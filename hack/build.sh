...
py_test() {
  local test_output=$(mktemp /tmp/skills-tickets-test-output.XXXXXX)
...
py_e2e() {
  local e2e_output=$(mktemp /tmp/skills-tickets-e2e-output.XXXXXX)
...
check_license() {
  ...
  local check_output=$(mktemp /tmp/skills-tickets-licence-check.XXXXXX)
